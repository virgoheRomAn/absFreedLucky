...
(function ($) {
    function Jbox() {
        ...
        this.error = function (cont, obj, icon) {
            var iconSrc = icon ? icon : "img/Jbox/Jerror.png";
        ...
        this.success = function (cont, obj, icon) {
            var iconSrc = icon ? icon : "img/Jbox/Jsuccess.png";
            preLoadImage(iconSrc, function () {
                tip(cont ? cont : "", obj, iconSrc);
            });
        };
        ...
        this.waring = function (cont, obj, icon) {
            var iconSrc = icon ? icon : "../img/Jbox/Jalert.png";
            preLoadImage(iconSrc, function () {
                tip(cont ? cont : "", obj, iconSrc);
            });
        };
        ...
        this.loading = function (cont, obj, icon) {
            var iconSrc = icon ? icon : "img/Jbox/Jloading.gif";
            preLoadImage(iconSrc, function () {
        ...
        this.confirm = function (cont, btn, obj, icon) {
            if (icon) {
                preLoadImage(icon, function () {
                    tipButton("confirm", cont, btn, obj, icon);
                });
            } else {
                tipButton("confirm", cont, btn, obj, icon);
            }
        ...
        this.btnAlert = function (cont, btn, obj, icon) {
            if (icon) {
                preLoadImage(icon, function () {
                    tipButton("alert", cont, btn, obj, icon);
                });
            } else {
                tipButton("alert", cont, btn ? btn : ["确定"], obj, icon);
            }
        };
        ...
        this.prompt = function (cont, btn, obj, icon) {
            if (icon) {
                preLoadImage(icon, function () {
                    tipButton("prompt", cont, btn, obj, icon);
                });
            } else {
                tipButton("prompt", cont, btn, obj, icon);
            }
        ...
        var tip = function (cont, obj, icon, type) {
            ...
            _html += '  <div class="' + cls + ' j-alert-ani animated" style="width:' + w + 'px; height:' + h + 'px; margin:' + -(h / 2) + 'px 0 0 ' + -(w / 2) + 'px;">';
            _html += '      <div class="jDisk-middle">';
            _html += iconStr;
        ...
        var tipButton = function (type, cont, obj, btn, icon) {
            ...
            _html += '  <div class="' + cls + ' j-alert-ani animated" style="width:' + w + 'px; height:' + h + 'px; margin:' + -(h / 2) + 'px 0 0 ' + -(w / 2) + 'px;">';
            _html += '      <div class="jDisk-middle" style="height:' + (h - 44) + 'px;">';
            _html += '          <p>';
            _html += contHTML;
            _html += '          </p>';
            _html += '      </div>';
    ...
    preLoadImage('img/Jbox/Jloading.gif', "");
    preLoadImage('img/Jbox/Jerror.png', "");
    preLoadImage('img/Jbox/Jsuccess.png', "");
    preLoadImage('img/Jbox/Jalert.png', "");

    preLoadImage('img/Jloading.gif', "");
    preLoadImage('img/Jerror.png', "");
    preLoadImage('img/Jsuccess.png', "");
    preLoadImage('img/Jalert.png', "");
    ...
    var style = '<style>' +
        '.jDisk-box{ position: absolute; width: 100%; height: 100%; top: 0; left: 0; z-index: 1000;}  ' +
        ...
        '.jDisk-middle > span{ display: block; padding: 8px; font-size: 12px; color: #FFFFFF; line-height: 16px;}' +
        '.jDisk-tips.jDisk-hasIcon{ padding:2px 0;}' +
        '.jDisk-tips.jDisk-hasIcon .jDisk-middle > span{ padding:5px 8px 0;}' +
        '.jDisk-tips.jDisk-hasIcon .jDisk-middle > span.noText{ padding:0;}' +
        '.jDisk-icon{ width:26px; height:26px; margin:auto;}' +
        ...
        '.jDisk-tips.jDisk-btnTips .jDisk-middle > p span{ display:block; padding:8px;}' +
        '.j-confirm-btn{ width: 100%; height: 44px; line-height: 44px; overflow: hidden; border-top: 1px solid #dcdada; position: absolute; bottom: 0; left: 0;}' +
