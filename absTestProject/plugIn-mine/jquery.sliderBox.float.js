...
(function ($) {
    ...
    $.slide = function (element, pattern, options) {
        ...
        that.init = function () {
            ...
            opt.container_height = opt.eleBox.height ? opt.eleBox.height : $(ele).outerHeight(true);
            ...
            //初始化回调
            if (opt.initFun) opt.initFun.call(ele, opt.item_length, opt.current);
        };
            ...
            switch (slidePattern) {
                    ...
                    if (dir) {
                        if (_is_loop_) {
                            $main.stop(true, true).animate({"left": slideOpt.cellSite * (cur + _clone_num_) * direction * moveDir}, an_time, ease, function () {
                                ...
                                if (callback) callback.call(child, callbackCur, maxLength);
                                _is_moving_ = false;
                            });
                        } else {
                ...
                case "fade":
                    $item.stop(true, true).eq(cur).animate({opacity: "show"}, an_time, ease, function () {
                        opt.current = cur;
                        if (callback) callback.call(child, opt.current, maxLength);
                        _is_moving_ = false;
                    }).siblings().hide();
                    break;
                case "marquee":
